 ...
 *   4. Обязательно обновить развёртывание, иначе по адресу /exec продолжит
 *      работать прежний код: «Начать развёртывание» → «Управление развёртываниями»
 *      → карандаш у активного → Версия: «Новая версия» → «Развернуть».
 *      Адрес при этом не меняется. Кнопка «Новое развёртывание» создаёт второй
 *      адрес — она не нужна.
 */
...
var STATUSES = ['Новая', 'Интересно, ждёт звонка', 'Готов на другие условия', 'В работе',
                'Ждём ответ клиента', 'Договор', 'Не подошло, не звонить', 'Отказ', 'Спам'];

// цвет строки: видно с одного взгляда, кому звонить, а кого не трогать
var STATUS_COLOR = {
  'Интересно, ждёт звонка'  : '#E7F6EC',   // зелёный — ждёт звонка
  'Готов на другие условия' : '#FFF4CC',   // жёлтый — цена не устроила, но обсуждать готов
  'Не подошло, не звонить'  : '#FCE4E4',   // красный — звонить не нужно
  'Договор'                 : '#DDF0E4',
  'Отказ'                   : '#F1F1F1',
  'Спам'                    : '#F1F1F1'
};
...
var SHEETS_CONFIG = {
  'Заявки': {
    headers:['Дата','Тип обращения','Форма / кнопка','Страница','Имя','Телефон','Email','Сообщение','Доп. данные','Рассылка','Статус','Ответственный','Заметки'],
    widths:[130,150,220,150,170,140,180,300,320,120,140,240],
    statusCol:11
  },
  'Калькулятор': {
    headers:['Дата','Имя','Компания','Телефон','Email','Сотрудники','Система','Ссылка на КП','Базовая','Стандарт','Оптима','№ КП','Рассылка','Статус','Ответственный','Заметки'],
    widths:[130,170,120,140,180,150,130,260,100,100,100,130,120,140,240],
    statusCol:14
  },
  'Работа у нас': {
    headers:['Дата','Имя','Телефон','Контакт','Направление','Опыт','Навыки','О себе','Резюме','Рассылка','Статус','Заметки'],
...
function addCalcFeedback(data) {
  ...
  // Сайт присылает одну запись с полной картиной, статус выводим по ней.
  var status;
  if (/готов рассмотреть другие условия/i.test(text)) {
    status = 'Готов на другие условия';       // цена не устроила, но разговор нужен
  } else if (/звонок не нужен/i.test(text)) {
    status = 'Не подошло, не звонить';        // прошёл расчёт, но звонить не просил
  } else if (/^интересно/i.test(text)) {
    status = 'Интересно, ждёт звонка';
  } else {
    status = 'Не подошло, не звонить';
  }
  sheet.getRange(target, cfg.statusCol).setValue(status);

  // подкрашиваем всю строку под статус
  var color = STATUS_COLOR[status] || null;
  sheet.getRange(target, 1, 1, cfg.headers.length).setBackground(color);
